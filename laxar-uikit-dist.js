System.register('lib/dom.js', [], function (_export) {/**
   ...
   //////////////////////////////////////////////////////////////////////////////////////////////////////////////
   return { setters: [], execute: function () {cssTransformPropertyName = createPrefixTest('transform', function (div, prefixedProperty) {return div.style[prefixedProperty] = 'rotate(45deg)';}, function (div, prefixedProperty) {return div.style[prefixedProperty] = '';}, function (div) {var rect = div.getBoundingClientRect();return rect.right - rect.left !== 100;});_export('cssTransformPropertyName', 
...
System.register('lib/formatter.js', ['laxar', 'moment'], function (_export) {/**
    ...
    *
    * @module formatter
    */'use strict';var ax, moment, 



   ISO_DATE_FORMAT, 
   ISO_TIME_FORMAT, 
   NUMERICAL_STRING_REGEXP, 



   formatters;_export('create', create);
   ...
   function create(type, optionalOptions) {
      ...
      return formatters[type].bind(formatters, options);}return { setters: [function (_laxar) {ax = _laxar;}, function (_moment) {moment = _moment['default'];}], execute: function () {ISO_DATE_FORMAT = 'YYYY-MM-DD';ISO_TIME_FORMAT = 'HH:mm:ss';NUMERICAL_STRING_REGEXP = /^(\+|\-)?\d+(\.\d+)?$/; //////////////////////////////////////////////////////////////////////////////////////////////////////////////
         formatters = { string: function string(options, str) {return str == null ? '' : '' + str;}, ///////////////////////////////////////////////////////////////////////////////////////////////////////////
            decimal: function decimal(options, number) {if (number == null) {return '';}if (typeof number === 'string' && NUMERICAL_STRING_REGEXP.exec(number)) {number = parseFloat(number);} else if (typeof number !== 'number') {throw new TypeError('Expected argument of type number, but got "' + typeof number + '". Value: ' + number);}var numberParts = toPrecision(number, options.decimalPlaces, options.decimalTruncation).split('.');var integerPart = numberParts[0].replace(/^[\-+]/, '');var fractionPart = numberParts[1] || '';var integerLength = integerPart.length - 1;var front = integerPart.split('').reduce(function (str, digit, index) {var pos = integerLength - index;return str + digit + (pos % 3 === 0 && pos !== 0 ? options.groupingSeparator : '');}, number < 0 ? '-' : '');return front + (fractionPart.length > 0 ? options.decimalSeparator + fractionPart : '');}, ///////////////////////////////////////////////////////////////////////////////////////////////////////////
            integer: function integer(options, number) {var integerOptions = ax.object.options({ decimalPlaces: 0 }, options);return formatters.decimal(integerOptions, number);}, ///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
System.register('lib/i18n.js', ['laxar', './moment_formats', './number_formats'], function (_export) {/**
   ...
   *
   * @module i18n
   */'use strict';var i18n, momentFormats, numberFormats, 
  ...
  function languageTagFromScope(scope) {
    return i18n.languageTagFromI18n(scope.$eval('i18n')) || DEFAULT_LANGUAGE_TAG;}
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////
  return { setters: [function (_laxar) {i18n = _laxar.i18n;}, function (_moment_formats) {momentFormats = _moment_formats['default'];}, function (_number_formats) {numberFormats = _number_formats['default'];}], execute: function () {DEFAULT_LANGUAGE_TAG = 'en';_export('languageTagFromScope', 

      languageTagFromScope);_export('momentFormatForLanguageTag', 
      momentFormatForLanguageTag);_export('numberFormatForLanguageTag', 
...
System.register('lib/parser.js', ['laxar', 'moment'], function (_export) {/**
    ...
    */_export('error', error);_export('create', create); //////////////////////////////////////////////////////////////////////////////////////////////////////////////
   function createDateTimeParser(formatKey, fallbackFormatsKey, isoFormat) {return function (options, str) {var val = ('' + str).trim();if (val.length === 0) {return success(null);}var formats = [options[formatKey]];if (options[fallbackFormatsKey] && options[fallbackFormatsKey].length) {formats = options[fallbackFormatsKey].concat(formats);}var mDate = momentParse(str, formats, options);return mDate.isValid() ? success(mDate.format(isoFormat)) : error();};} //////////////////////////////////////////////////////////////////////////////////////////////////////////////
   function momentParse(str, formats, options) {var origParseTwoDigitYear = moment.parseTwoDigitYear;if (options.dateTwoDigitYearWrap > -1 && options.dateTwoDigitYearWrap < 100) {moment.parseTwoDigitYear = function (input) {var int = parseInt(input, 10);int = isNaN(int) ? 0 : int;return int + (int > options.dateTwoDigitYearWrap ? 1900 : 2000);};}try {return moment(str, formats, true);} finally {moment.parseTwoDigitYear = origParseTwoDigitYear;}} //////////////////////////////////////////////////////////////////////////////////////////////////////////////
   /**
    ...
    */function success(val) {return { ok: true, value: val };}function error() {return { ok: false };}function create(type, optionalOptions) {var options = ax.object.options(optionalOptions, { groupingSeparator: ',', decimalSeparator: '.', dateFormat: 'M/D/YYYY', dateFallbackFormats: ['M/D/YY'], dateTwoDigitYearWrap: 68, timeFormat: 'H:m', timeFallbackFormats: ['H', 'HHmm'] });return parsers[type].bind({}, options);}return { setters: [function (_laxar) {ax = _laxar;}, function (_moment) {moment = _moment['default'];}], execute: function () {ISO_DATE_FORMAT = 'YYYY-MM-DD';ISO_TIME_FORMAT = 'HH:mm:ss';NUMERIC_VALUE_MATCHER = /^[+\-]?[0-9]*(\.[0-9]*)?$/; //////////////////////////////////////////////////////////////////////////////////////////////////////////////
         parsers = { string: function string(options, str) {return success(str);}, ///////////////////////////////////////////////////////////////////////////////////////////////////////////
            decimal: function decimal(options, str) {var val = ('' + str).trim();if (val.length === 0) {return success(null);}if (options.groupingSeparator) {while (val.indexOf(options.groupingSeparator) !== -1) {val = val.replace(options.groupingSeparator, '');}}val = val.replace(options.decimalSeparator, '.');if (NUMERIC_VALUE_MATCHER.exec(val)) {var parsed = parseFloat(val);if (!isNaN(parsed)) {return success(parsed);}}return error();}, ///////////////////////////////////////////////////////////////////////////////////////////////////////////
            integer: function integer(options, str) {var result = parsers.decimal(options, str);if (!result.ok || result.value === null) {return result;}var integerValue = Math.round(result.value);if (result.value !== integerValue) {return error();}return success(integerValue);}, ///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
System.register('laxar-uikit.js', ['./lib/dom', './lib/formatter', './lib/i18n', './lib/parser'], function (_export) {/**
                                                                                                                       * Copyright 2014 aixigo AG
                                                                                                                       * Released under the MIT license.
                                                                                                                       * http://laxarjs.org/license
                                                                                                                       */'use strict';var dom, formatter, i18n, parser;return { setters: [function (_libDom) {dom = _libDom;}, function (_libFormatter) {formatter = _libFormatter;}, function (_libI18n) {i18n = _libI18n;}, function (_libParser) {parser = _libParser;}], execute: function () {_export('dom', 
      ...
      dom);_export('formatter', 
      formatter);_export('i18n', 
      i18n);_export('parser', 
      parser);} };});
