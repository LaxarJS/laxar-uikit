...
describe( 'A formatter', () => {
      ...
      it( 'should throw a type error for non ISO-8601-like date strings', () => {
         expect( () => { format( true ); } )
            .toThrow( new TypeError(
               'Expected argument as ISO-8601 date string of the form YYYY-MM-DD, but got "boolean". ' +
               'Value: true'
            ) );
         expect( () => { format( 'Hello' ); } )
            .toThrow( new TypeError(
               'Expected argument as ISO-8601 date string of the form YYYY-MM-DD, ' +
               'but got "string". Value: Hello'
            ) );
         expect( () => { format( new Date() ); } ).toThrow(); // Spare testing for the date string ...
      } );
   ...
   describe( 'for time', () => {
      ...
      it( 'should throw a type error for non ISO-8601-like time strings', () => {
         expect( () => { format( true ); } )
            .toThrow( new TypeError(
               'Expected argument as ISO-8601 time string of the form HH:mm:ss, but got "boolean". ' +
               'Value: true'
            ) );
         expect( () => { format( 'Hello' ); } )
            .toThrow( new TypeError(
               'Expected argument as ISO-8601 time string of the form HH:mm:ss, but got "string". ' +
               'Value: Hello'
            ) );
         expect( () => { format( new Date() ); } ).toThrow(); // Spare testing for the date string ...
      } );
