...
describe( 'A formatter', () => {
   ...
   describe( 'for date', () => {
      ...
      it( 'should throw a type error for non ISO-8601-like date strings', () => {
         expect( () => { format( true ); } )
            .toThrow( new TypeError(
               ...
               '(full, or date as YYYY-MM-DD), but got "boolean". Value: true'
            ) );
         expect( () => { format( 'Hello' ); } )
            .toThrow( new TypeError(
               'Expected argument to be a Date object, the string "NOW", or an ISO-8601 string ' +
               '(full, or date as YYYY-MM-DD), but got "string". Value: Hello'
            ) );
      } );
      ...
      it( 'should format ISO-8601-like date strings to the given format', () => {
         expect( format( '1962-01-04T21:01:01.333Z' ) ).toEqual( '04.01.1962' );
         expect( format( '1962-01-04' ) ).toEqual( '04.01.1962' );
      } );

      ////////////////////////////////////////////////////////////////////////////////////////////////////////

      it( 'should format the date-portion of Date objects to the given format', () => {
         const now = new Date();
         expect( format( 'NOW' ) ).toEqual( testFormat( now ) );
      } );

      ////////////////////////////////////////////////////////////////////////////////////////////////////////

      it( 'should format the date-portion of "NOW" (current date) to the given format', () => {
         const now = new Date();
         expect( format( 'NOW' ) ).toEqual( testFormat( now ) );
      } );
   ...
   describe( 'for time', () => {
      ...
      it( 'should throw a type error for non-ISO-8601-like values', () => {
         expect( () => { format( true ); } )
            .toThrow( new TypeError(
               'Expected argument to be a Date object, the string "NOW", or an ISO-8601 string ' +
               '(full, or time as HH:mm:ss), but got "boolean". Value: true'
            ) );
         expect( () => { format( 'Hello' ); } )
            .toThrow( new TypeError(
               'Expected argument to be a Date object, the string "NOW", or an ISO-8601 string ' +
               '(full, or time as HH:mm:ss), but got "string". Value: Hello'
            ) );
      } );
      ...
      it( 'should format ISO-8601-like date strings to the given format', () => {
         expect( format( '2011-02-03T11:23:45.123' ) ).toEqual( '11:23:45' );
      ...
      it( 'should format the time-portion of Date objects to the given format', () => {
         const now = new Date();
         expect( format( now ) ).toEqual( testFormat( now ) );
      } );
      ...
      it( 'should format the time-portion of "NOW" (current date) to the given format', () => {
         const now = new Date();
         expect( format( 'NOW' ) ).toEqual( testFormat( now ) );
      } );
