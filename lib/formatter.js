...
const formatters = {
   ...
   decimal( options, number ) {
      ...
      if( typeof number === 'string' && NUMERICAL_STRING_REGEXP.exec( number ) ) {
         ...
         throw new TypeError(
            `Expected argument of type number, but got "${typeof number}". Value: ${number}`
         );
      }

      const numberParts = toPrecision( number, options.decimalPlaces, options.decimalTruncation ).split( '.' );
   ...
   integer( options, number ) {
      const integerOptions = ax.object.options( { decimalPlaces: 0 }, options );
...
export function create( type, optionalOptions ) {
   const options = ax.object.options( optionalOptions, {
      groupingSeparator: ',',
      decimalSeparator: '.',
      decimalPlaces: 2,
